...
const zh = {
  ...
  portal: {
    ...
    messages: {
      ...
      localNodeBlocked: "电脑助手在运行，但浏览器拦住了网页访问它。若浏览器弹出“允许访问本地网络”，请点【允许】；没弹窗就在地址栏打开 http://127.0.0.1:8790/health 自测（能打开=助手正常）。",
      localNodeNotDetected: "检测不到电脑助手。请确认：① 助手已打开；② 浏览器提示“允许访问本地网络”时点【允许】。自测：浏览器打开 http://127.0.0.1:8790/health —— 能打开=助手在运行（请允许本地网络访问）；打不开=助手没启动，请重开，或把它加进杀毒软件白名单。",
...
const en: Messages = {
  ...
  portal: {
    ...
    messages: {
      ...
      localNodeBlocked: "The helper is running, but the browser blocked the page from reaching it. Click Allow on the “access local network” prompt; if there is no prompt, open http://127.0.0.1:8790/health in your browser to self-test (loads = helper is fine).",
      localNodeNotDetected: "Helper not detected. Make sure ① it is open and ② you clicked Allow on the “access local network” prompt. Self-test: open http://127.0.0.1:8790/health — loads = helper running (allow local network access); doesn’t load = helper not started (reopen it, or whitelist it in your antivirus).",
